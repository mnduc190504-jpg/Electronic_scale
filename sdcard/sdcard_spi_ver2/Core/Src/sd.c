...
uint8_t sd_ini(void)
{
  ...
  sdinfo.type = 0;				//Kieu the
	...
	//hspi1.Init.BaudRatePrescaler = SPI_BAUDRATEPRESCALER_128; //156.25 kbps
		hspi1.Init.BaudRatePrescaler = SPI_BAUDRATEPRESCALER_256; //// 78.125 kbps
	HAL_SPI_Init(&hspi1);
	SS_SD_DESELECT();
...
FRESULT ReadLongFile(void)
{
  ...
  do
  {
    ...
    for(i=0;i<bytesread;i++)
    {
      ...
      HAL_UART_Transmit(&huart6,SD.rdata+i,1,0x1000);
    }
    ind+=i1;
  }
  while(f_size>0);
		...
		if (result == FR_OK)
		{
			while(1)
			{
				...
				if (result==FR_OK && fileInfo.fname[0])
				{
					...
					if(strlen(fn)) HAL_UART_Transmit(&huart6,(uint8_t*)fn,strlen(fn),0x1000);
					else HAL_UART_Transmit(&huart6,(uint8_t*)fileInfo.fname,strlen((char*)fileInfo.fname),0x1000);
					if(fileInfo.fattrib&AM_DIR)
					{
						HAL_UART_Transmit(&huart6,(uint8_t*)"  [DIR]",7,0x1000);
					}					
				}
				else break;
				HAL_UART_Transmit(&huart6,(uint8_t*)"\r\n",2,0x1000);
